...
class BoardOutException(BoardException):
    def __str__(self):
        return "За пределы игрового поля"


class BoardUsedException(BoardException):
    def __str__(self):
        return "Вы уже попали в эту точку"
...
class AI(Player):
    def ask(self):
        ...
        print(color.GREEN + f"Ход Компьютера: {d.x + 1} {d.y + 1}" + color.END)
        return d
...
class User(Player):
    def ask(self):
        while True:
            cords = input(color.BLUE + "Ваш ход: " + color.END).split()
...
class Game:
    ...
    def loop(self):
        ...
        while True:
            ...
            if num % 2 == 0:
                print(color.BLUE + "Ходит Игрок!" + color.END)
                repeat = self.us.move()
            else:
